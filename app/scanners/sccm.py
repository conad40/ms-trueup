...
async def scan(pool, settings):
    """Scan SCCM for devices. Returns (scanned, failed, errors)."""
    base_url = settings.get("sccm_server_url", "").rstrip("/")
    username = settings.get("sccm_username", "")
    password = settings.get("sccm_password", "")
    verify_ssl = settings.get("sccm_verify_ssl", "false").lower() == "true"

    if not base_url or not username:
        log.warning("SCCM not configured")
        return 0, 0, []
    ...
    async with httpx.AsyncClient(auth=auth, verify=verify_ssl, timeout=60) as client:
        ...
        for device in devices:
            ...
            try:
                ...
                log.warning(f"SCCM failed for {hostname}: {e}")
